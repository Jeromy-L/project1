...
function debug(str) {
//	if($("#debug").length == 0){
//		$("<div id='debug' style='position:absolute;font-size:20px;color:red;'></div>").appendTo(document.body);
//	}
//	$("#debug").html($("#debug").html() + str);
}

function coocaakeymap(buts, curlink, hover, getVal, setVal){
	...
	//若有hover选择器则使用hover选择器，若无默认为hover
	...
	$(document).unbind("keydown").bind('keydown', function(ev){_this.keyHandler(_this, ev);});
	this.cmd = [];
	this.iscmd = false;
	this.debugCmd = {
...
coocaakeymap.prototype.keyHandler = function(_this, ev){
	...
	debug("<br/>");
	debug("keyCode = " + ev.keyCode + ",case by:" + ev.target.nodeName.toLowerCase());
	debug("<br/>");
	
	
//	if(ev.target.nodeName.toLowerCase() == "#document"){ 
//		return;
//	}
	...
	if(ev.isPropagationStopped() == true){
		return;
	}
	//禁止select 左右上下 改变选项
...
coocaakeymap.prototype.moveRight = function(){
	...
	if(_this.curLink.attr("rightTarget")){
		if($(_this.curLink.attr("rightTarget")).length==0)
		{
			return "";
		}
...
coocaakeymap.prototype.moveDown = function(){
	...
	if(_this.curLink.attr("downTarget")){
		if($(_this.curLink.attr("downTarget")).length==0)
		{
			return ;
		}
